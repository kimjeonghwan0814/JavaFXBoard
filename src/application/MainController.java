...
public class MainController implements Initializable {
	...
	@FXML
	private ListView<String> listView_menu;
	@FXML
	private TableView tableView_boardList;
	@FXML
	...
	public void setListView() {
		listView_menu.getItems().add("BOARD1");
		listView_menu.getItems().add("BOARD2");
		listView_menu.getItems().add("BOARD3");
		
		// 테이블뷰에 마우스 클릭 이벤트를 추가한다.
		listView_menu.setOnMousePressed(new EventHandler<MouseEvent>() {	// 익명클래스
			@Override
			public void handle(MouseEvent event) {
				// 이벤트가 마우스 주버튼 클릭이고, 마우스 클릭수가 1번이면 더블클릭이라고 판단
				if (event.isPrimaryButtonDown()) {
					
					boardType = listView_menu.getSelectionModel().getSelectedItem();
//					tableView_boardList.refresh();
//					tableView_boardList.
					setTableData(null, null);
				}
			}
	...
	public void setTableData(String boardTitle, String boardWriter) {
		...
		// 게시판 리스트를 불러옴
		List<BoardVO> list = biz.selectBoardList(boardTitle, boardWriter, boardType);
		// 테이블뷰에 바인드 될 옵저버블리스트
